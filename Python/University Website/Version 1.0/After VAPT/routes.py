...
def route(app, db):
    ...
    def register_page():
        ...
        if form.validate_on_submit():
            student_to_create = application.Student(id=form.roll_number.data, name=form.name.data,stream=form.stream.data,
                                                    d_o_b=form.d_o_b.data, email_address=form.email_address.data,
                                                    username=form.username.data, password=form.password1.data)
            # db.session.add(student_to_create)
            # convert_password = application.password(form.password1.data)
            r = db.engine.execute('insert into Student (id,name,stream,d_o_b,email_address,username,password_hash) values (?,?,?,?,?,?,?)',form.roll_number.data, form.name.data, form.stream.data, form.d_o_b.data, form.email_address.data, form.username.data, application.password(form.password1.data))
            db.session.commit()
